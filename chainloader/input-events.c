...
efi_main (EFI_HANDLE image_handle, EFI_SYSTEM_TABLE *sys_table)
{
    ...
    WARN_STATUS( res, L"No filesystem associated with bootloader\n" );

    res = efi_mount( fs, &root_dir );
    WARN_STATUS( res, L"Unable to mount bootloader filesystem\n" );

    loader_file = get_self_file();
    orig_path = device_path_string( loader_file );
    debug_log_init ( root_dir, orig_path, L"input-debug.log" );

    v_msg( L"INPUT EVENTS\n" );
    ...
    if( EFI_ERROR( res ) )
    {
        v_msg( L"  area: 80 x 25 (fallback values: %r)\n", res );
        cols = 80;
        rows = 25;
    }
    else
    ...
    v_msg( L"Starting input event loop\n" );
